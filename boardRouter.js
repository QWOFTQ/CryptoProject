...
router.use('/', (req, res, next) => {     // 메인 페이지
  const { user } = req.session;     // 세션에 저장되어 있는 유저 데이터
  if (user !== undefined) {     // user 데이터가 있는 경우 게시판 확인 가능
    next();
  } else {
...
router.get('/list', (req, res) => {   // 게시판
  const { user } = req.session;
  res.render('../views/board/list', {     // views/board/list 페이지 글목록, 유저 데이터 포함해서 보여주기
    list,
    user,
  });
...
router.get('/update', (req, res) => {   // 수정
  const { user } = req.session;
  const index = req.query.index;
  console.log(list);
  if (list[index - 1].userid === user.userid) {     // 유저 아이디 같은 경우 수정 
